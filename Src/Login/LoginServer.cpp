                                      ...
                                      const std::string& clientIP)
{
    ...
    DB::DBWorkerPool::Instance().AsyncQuery(
        ...
        {DB::DBValue(username)},
        [this, socket = std::move(socket), clientIP,
         username = std::move(username), password = std::move(password),
         clientDHKey = std::move(clientDHKey)](const DB::DBResult& result)
        {
            OnAuthDBCallback(std::move(socket), clientIP,
                             username, password, clientDHKey, result);
        });
}
                                    ...
                                    const DB::DBResult& result)
{
    ...
    if (clientDHKey.size() != Crypto::EcdhX25519::kKeySize)
    {
        Log::Error("LoginServer: client DH key size mismatch (got {}, expected {})",
                    clientDHKey.size(), Crypto::EcdhX25519::kKeySize);
        asio::post(socket->Socket().get_executor(),
            [this, socket, errorCode = static_cast<uint32>(1003)]()
            {
                SendAuthFailure(socket, errorCode);
    ...
    auto tokenOpt = Crypto::SessionTokenBuilder::Issue(
        _lss,
        sessionKeyBuf.Data(),
        _worldServerID,
    ...
    asio::post(socket->Socket().get_executor(),
        [this, socket, keyPair = std::move(keyPair),
         token = std::move(*tokenOpt)]() mutable
        {
...
void LoginServer::SendAuthFailure(std::shared_ptr<TCPSocket> socket,
                                   uint32 errorCode)
{
    LoginAuthRsp rsp;
    auto* error = rsp.mutable_error();
