...
static das::TArray<uint64> Bridge_EntitiesInRadius(const das::float3 &center, float radius)
{
...
static BattleStats *Bridge_EntityGetBattleStats(uint64 fullEntityId)
{
    ...
    auto *stats = ctx ? reinterpret_cast<BattleStats *>(ctx->allocate(sizeof(BattleStats))) : nullptr;
    if (!stats)
    {
        return nullptr;
    }
...
static void Bridge_SendToClient(uint32 sessionID, uint32 msgID, const das::TArray<uint8> &data)
{
...
static uint32 Bridge_ScheduleTimer(int32 delayMs, const das::TBlock<void, uint32> &block)
{
    ...
    mod._timingWheel->Schedule(std::chrono::milliseconds(delayMs), [timerID, ctx, &mod]() {
        ...
        if (it != mod._timerCallbacks.end())
        {
            ...
            das_invoke<void>::invoke(ctx.get(), nullptr, it->second.block, timerID);
...
namespace MMO
{
    ...
    void MassiveModule::BindFunctions()
    {
        ...
        // ── 3.1 空间查询 ──
        addExtern<DAS_BIND_FUN(Bridge_EntityPosition)>(*this, lib, "EntityPosition", SideEffects::none);
        ...
        // ── 3.3 Tag 判断 ──
        addExtern<DAS_BIND_FUN(Bridge_EntityIsDead)>(*this, lib, "EntityIsDead", SideEffects::none);
        addExtern<DAS_BIND_FUN(Bridge_EntityIsInCombat)>(*this, lib, "EntityIsInCombat", SideEffects::none);
        addExtern<DAS_BIND_FUN(Bridge_EntityIsStunned)>(*this, lib, "EntityIsStunned", SideEffects::none);
        addExtern<DAS_BIND_FUN(Bridge_EntityIsPlayer)>(*this, lib, "EntityIsPlayer", SideEffects::none);
        addExtern<DAS_BIND_FUN(Bridge_EntityIsMonster)>(*this, lib, "EntityIsMonster", SideEffects::none);

        // ── 3.4 世界交互 ──
        addExtern<DAS_BIND_FUN(Bridge_CreateEntity)>(*this, lib, "CreateEntity", SideEffects::modifyExternal);
        addExtern<DAS_BIND_FUN(Bridge_DestroyEntity)>(*this,
                                                      lib,
                                                      "DestroyEntity",
                                                      SideEffects::modifyExternal);
        addExtern<DAS_BIND_FUN(Bridge_SendToClient)>(*this, lib, "SendToClient", SideEffects::modifyExternal);

        // ── 3.5 定时器 ──
        addExtern<DAS_BIND_FUN(Bridge_ScheduleTimer)>(*this,
                                                      lib,
                                                      "ScheduleTimer",
                                                      SideEffects::modifyExternal);
        addExtern<DAS_BIND_FUN(Bridge_CancelTimer)>(*this, lib, "CancelTimer", SideEffects::modifyExternal);

        // ── 3.6 工具函数 ──
        addExtern<DAS_BIND_FUN(Bridge_GetDeltaTime)>(*this, lib, "GetDeltaTime", SideEffects::none);
        addExtern<DAS_BIND_FUN(Bridge_FindEntityBySession)>(*this,
                                                            lib,
                                                            "FindEntityBySession",
                                                            SideEffects::none);

        // ── 日志 ──
        addExtern<DAS_BIND_FUN(Bridge_LogInfo)>(*this, lib, "LogInfo", SideEffects::modifyExternal)
            ->args({"text", "context", "at"});
        addExtern<DAS_BIND_FUN(Bridge_LogWarn)>(*this, lib, "LogWarn", SideEffects::modifyExternal)
            ->args({"text", "context", "at"});
        addExtern<DAS_BIND_FUN(Bridge_LogError)>(*this, lib, "LogError", SideEffects::modifyExternal)
            ->args({"text", "context", "at"});
