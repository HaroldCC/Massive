...
public:
    ...
    Range()
        : _tableName(Table::kTableName)
    {
    }

    /** @brief 显式指定 DBWorkerPool */
    explicit Range(DBWorkerPool* pool)
        : _tableName(Table::kTableName)
        , _pool(pool)
    {
    }
    ...
    void ToArray(std::function<void(std::vector<RowType>)> callback)
    {
        ...
        pool->AsyncQuery(std::move(sql), std::move(params),
            [cb = std::move(callback)](const DBResult& res)
            {
                std::vector<RowType> rows;
                if (!res.IsOK()) { cb(std::move(rows)); return; }
...
private:
    void AddExpr(const Expression<int32_t>& expr)      { _int32Exprs.push_back(expr); }
    void AddExpr(const Expression<int64_t>& expr)      { _int64Exprs.push_back(expr); }
    void AddExpr(const Expression<float>& expr)        { _floatExprs.push_back(expr); }
    void AddExpr(const Expression<double>& expr)       { _doubleExprs.push_back(expr); }
    void AddExpr(const Expression<std::string>& expr)  { _stringExprs.push_back(expr); }
    void AddExpr(const CompoundExpr& expr)             { _compoundExprs.push_back(expr); }
    ...
    std::string BuildWhereSQL(std::vector<DBValue>& params) const
    {
        ...
        auto appendExpr = [&params, &result](const auto& expr, int& paramIdx) -> void
        {
            ...
            switch (expr.op)
            {
            case EOp::EQ: result += " = $"; break;
            case EOp::NE: result += " <> $"; break;
            case EOp::LT: result += " < $"; break;
            case EOp::LE: result += " <= $"; break;
            case EOp::GT: result += " > $"; break;
            case EOp::GE: result += " >= $"; break;
            }
        ...
        for (auto& expr : _int32Exprs)    { appendExpr(expr, paramIdx); }
        for (auto& expr : _int64Exprs)    { appendExpr(expr, paramIdx); }
        for (auto& expr : _floatExprs)    { appendExpr(expr, paramIdx); }
        for (auto& expr : _doubleExprs)   { appendExpr(expr, paramIdx); }
        for (auto& expr : _stringExprs)   { appendExpr(expr, paramIdx); }
