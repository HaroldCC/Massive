...
public:
    ...
    const char* CStr() const { return _isNull ? nullptr : _text.c_str(); }
    // 用于 PQexecParams 的长度数组
    int Length() const { return _isNull ? 0 : static_cast<int>(_text.size()); }
...
public:
    ...
    int AffectedRows() const { return _affectedRows; }
