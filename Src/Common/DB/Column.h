...
struct CompoundExpr
{
    ...
    template <typename T>
    CompoundExpr(const Expression<T>& a, const Expression<T>& b, ELogicalOp op)
        : logicalOp(op)
    {
        Add(a);
        Add(b);
    ...
    CompoundExpr operator&&(const CompoundExpr& other) const
    {
        ...
        result.int32Exprs.insert(result.int32Exprs.end(),
            other.int32Exprs.begin(), other.int32Exprs.end());
        result.int64Exprs.insert(result.int64Exprs.end(),
            other.int64Exprs.begin(), other.int64Exprs.end());
        result.floatExprs.insert(result.floatExprs.end(),
            other.floatExprs.begin(), other.floatExprs.end());
        result.doubleExprs.insert(result.doubleExprs.end(),
            other.doubleExprs.begin(), other.doubleExprs.end());
        result.stringExprs.insert(result.stringExprs.end(),
            other.stringExprs.begin(), other.stringExprs.end());
        return result;
    }
...
struct Column
{
    ...
    constexpr auto operator==(T value) const { return Expression<T>{name, EOp::EQ, value}; }
    constexpr auto operator!=(T value) const { return Expression<T>{name, EOp::NE, value}; }
    constexpr auto operator<(T value)  const { return Expression<T>{name, EOp::LT, value}; }
    constexpr auto operator<=(T value) const { return Expression<T>{name, EOp::LE, value}; }
    constexpr auto operator>(T value)  const { return Expression<T>{name, EOp::GT, value}; }
    constexpr auto operator>=(T value) const { return Expression<T>{name, EOp::GE, value}; }
};
...
public:
    ...
    const auto& Assignments() const { return _assignments; }
