 ...
 *      零运行期开销。
 *   2. 异步日志（overrun_oldest 策略），不阻塞逻辑线程。
 *
 * 使用示例：
 * @code
 ...
 */
template <typename... Args>
void LogDispatchAt(ELogLevel level, const char* file, int line, std::string_view fmtStr, Args&&... args)
{
