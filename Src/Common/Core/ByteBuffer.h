...
public:
    ...
    uint16 ReadUint16()
    {
        ...
        uint16 result = static_cast<uint16>(
            (static_cast<uint16>(_data[_readPos]) << 8)
    ...
    uint32 ReadUint32()
    {
        CheckRead(4);
        uint32 result =
            (static_cast<uint32>(_data[_readPos]) << 24)
    ...
    uint64 ReadUint64()
    {
        CheckRead(8);
        uint64 result =
            (static_cast<uint64>(_data[_readPos]) << 56)
    ...
    /** @brief 当前有效数据长度 */
    size_t Size() const { return _writePos; }
    /** @brief 剩余可读字节数 */
    ...
    const uint8* ReadPtr() const { return _data + _readPos; }
    uint8* WritePtr() { return _data + _writePos; }

    // ── 位置操作 ──

    /** @brief 跳过读取指定字节数 */
    void SkipRead(size_t bytes) { CheckRead(bytes); _readPos += bytes; }
    /** @brief 重置读取位置 */
    void ResetRead() { _readPos = 0; }
    /** @brief 重置读写位置 */
    void ResetWrite() { _writePos = 0; _readPos = 0; }

    /** @brief 手动设置写入位置 */
    void SetWritePos(size_t pos) { _writePos = pos; }
    ...
    /** @brief 是否拥有内存所有权 */
    bool IsOwner() const { return _ownsMemory; }
