...
{
    MASSIVE_ASSERT(_readPos + count <= _writePos,
        "ByteBuffer: read beyond write position");
}
...
{
    MASSIVE_ASSERT(_ownsMemory,
        "ByteBuffer: write on non-owned buffer (Wrap mode is read-only)");
