...
public:
    ...
    uint32 SceneID() const { return _sceneId; }
    ...
    {
        return _registry.emplace<T>(entt::entity(entity.entityId),
     ...
     * @brief 获取脚本组件存储
     * @param name  组件名
     * @return ScriptComponentStorage*，未注册返回 nullptr
     */
    ScriptComponentStorage* GetScriptStorage(const std::string& name);

    const std::unordered_map<std::string, std::unique_ptr<ScriptComponentStorage>>&
        ScriptStorages() const { return _scriptStorages; }

private:
    uint32 _sceneId;
    entt::registry _registry;
    std::unordered_map<std::string, std::unique_ptr<ScriptComponentStorage>> _scriptStorages;
};
