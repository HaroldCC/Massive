...
namespace MMO::Crypto
{
     ...
     */
    std::optional<SessionToken> SessionTokenBuilder::Issue(const uint8 *lss,
     ...
     */
    std::optional<SessionTokenBuilder::TokenPayload> SessionTokenBuilder::Verify(const uint8        *lss,
                                                                                 const SessionToken &token)
    {
        auto expectedHmac = HmacSha256::Sign(lss,
                                             Aes256Ecb::kKeySize,
                                             token.data,
                                             SessionToken::kPlainSize + SessionToken::kKeyEncSize);
