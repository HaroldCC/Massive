...
namespace MMO::Crypto
{
    ...
    HmacSha256::Sign(const uint8 *key, size_t keyLen, const uint8 *data, size_t dataLen)
    {
        auto out = ByteBuffer::Own(kHashSize);

        unsigned int outLen = 0;
        uint8 *result = HMAC(EVP_sha256(), key, static_cast<int>(keyLen), data, dataLen, out.Data(), &outLen);
