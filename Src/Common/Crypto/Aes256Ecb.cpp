...
namespace MMO::Crypto
{
     ...
     */
    std::optional<ByteBuffer> Aes256Ecb::Encrypt(const uint8 *key, const uint8 *plaintext, size_t len)
    {
        MASSIVE_ASSERT(len % kBlockSize == 0, "Aes256Ecb::Encrypt: data length must be multiple of 16");
     ...
     */
    std::optional<ByteBuffer> Aes256Ecb::Decrypt(const uint8 *key, const uint8 *ciphertext, size_t len)
    {
        MASSIVE_ASSERT(len % kBlockSize == 0, "Aes256Ecb::Decrypt: data length must be multiple of 16");
