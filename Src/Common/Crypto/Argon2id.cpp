...
std::string Argon2id::HashPassword(std::string_view password)
{
    ...
    unsigned char out[kHashLen];
    {
        ...
        OSSL_PARAM params[] = {
            OSSL_PARAM_construct_octet_string(kPass, const_cast<char*>(password.data()), password.size()),
...
bool Argon2id::VerifyPassword(std::string_view password, std::string_view storedHash)
{
    ...
    if (storedHash.size() != expectedLen) return false;
    ...
    for (int i = 0; i < kHashLen; ++i)
    {
        ...
        expected[i] = static_cast<unsigned char>((HexDigit(storedHash[pos]) << 4)
                                                  | HexDigit(storedHash[pos + 1]));
    }
