 ...
 */
std::optional<ByteBuffer> Aes256Gcm::Encrypt(
    const uint8* key,
    const uint8* iv,
    ...
    size_t plaintextLen)
{
    ...
    if (EVP_EncryptUpdate(ctx.Get(), out.WritePtr(), &outLen, plaintext, static_cast<int>(plaintextLen)) != 1)
    {
        return std::nullopt;
    }
 ...
 */
std::optional<ByteBuffer> Aes256Gcm::Decrypt(
    const uint8* key,
    const uint8* iv,
    ...
    size_t ciphertextLen)
{
    ...
    if (EVP_DecryptUpdate(ctx.Get(), out.WritePtr(), &outLen, ciphertext, static_cast<int>(actualCipherLen)) != 1)
    {
        return std::nullopt;
    }
    out.SetWritePos(outLen);
