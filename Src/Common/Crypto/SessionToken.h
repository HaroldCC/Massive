...
struct SessionToken
{
    ...
    void SetWorldServerId(uint16 id) { std::memcpy(data, &id, 2); }
    /** @brief 设置 AccountId */
    void SetAccountId(uint32 id) { std::memcpy(data + 2, &id, 4); }
    /** @brief 设置过期时间 */
    void SetExpireTime(uint32 t) { std::memcpy(data + 6, &t, 4); }

    // ── 加密段 ──

    const uint8* EncryptedSessionKey() const { return data + kPlainSize; }
    uint8* EncryptedSessionKey() { return data + kPlainSize; }

    // ── HMAC 段 ──

    const uint8* Hmac() const { return data + kPlainSize + kKeyEncSize; }
    uint8* Hmac() { return data + kPlainSize + kKeyEncSize; }
...
public:
     ...
     */
    static std::optional<SessionToken> Issue(
        const uint8* lss,
     ...
     */
    static std::optional<TokenPayload> Verify(
        const uint8* lss,
